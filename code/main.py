...
#save_path = join(world.BOARD_PATH,world.config['dataset'] +"-T" + str(world.config['temperature'])+"L"+str(world.config['lightGCN_n_layers'])+time.strftime("%m-%d-%Hh%Mm%Ss-") +".txt") 
save_path = join(world.BOARD_PATH,world.config['dataset'] +"-T" + str(world.config['temperature'])+time.strftime("%m-%d-%Hh%Mm%Ss-") +".txt") 
...
try:
    for epoch in range(world.TRAIN_epochs):
        ...
        if  ("bpr" in world.config['info'] and  epoch//10 == 0) or ("bpr"  not in world.config['info'] and epoch > 0):  
            cprint("[TEST]")
